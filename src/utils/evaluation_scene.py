...
def evaluate(test_annotation_file: str, user_submission_file: str, phase_codename: str = 'Multispectral'):
    ...
    eval_result['campus'].accumulate()
    MR_campus = eval_result['campus'].summarize(0)

    eval_result['road'].params.imgIds = _road
    eval_result['road'].evaluate(0)
    eval_result['road'].accumulate()
    MR_road = eval_result['road'].summarize(0)

    eval_result['downtown'].params.imgIds = _downtown
    eval_result['downtown'].evaluate(0)
    eval_result['downtown'].accumulate()
    MR_downtown = eval_result['downtown'].summarize(0)

    title_str = f'\n########## Method: {method} ##########\n'
    msg = title_str \
        + f'MR_campus  : {MR_campus * 100:.2f}\n' \
        + f'MR_road    : {MR_road * 100:.2f}\n' \
        + f'MR_downtown: {MR_downtown * 100:.2f}\n' \
        + '#'*len(title_str) + '\n\n'
    print(msg)
