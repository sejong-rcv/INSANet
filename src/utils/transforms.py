...
__all__ = [ "Compose", "ToTensor", "ToPILImage", "Normalize", "UnNormalize", "Stretch", "Resize", "Lambda", 
            "RandomHorizontalFlip", "TT_RandomHorizontalFlip", "TT_FixedHorizontalFlip",  "RandomResizedCrop", "TT_RandomResizedCrop", 
            "TT_RandomHorizontalShift", "PI_RandomHorizontalShift", "TT_RandomShift", "ColorJitter", "ColorJitterLWIR"]
...
class Compose(object):
    ...
    def __call__(self, img, mask=None, vis_box=None, lwir_box=None, pair=1):
        ...
        for t in self.transforms:
            if self.args != None and t.__class__.__name__ in self.args.augmentation:
...
class TT_RandomHorizontalShift(object):
    """Horizontally (x-axis) shift the given PIL Image randomly with a given probability.
    Args:
        p (float): probability of the image being shifted. Default value is 0.5
        t (float or int): max shift pixel choosed randomly.
    """
    def __init__(self, p: float =0.5, x: int =0):
        self.p = p
        self.x = x
        
    def __call__(self, img, ann=None, box_vis=None, box_lwir=None, pair=None):
        """
        :param img (PIL Image): Image to be shifted.
        :return: Randomly shifted image.
        """
        
        if random.random() < self.p:
            x = random.randint(self.x, 0) \
                if self.x < 0 else random.randint(0, self.x)
                
            M = np.float64([[1, 0, x], [0, 1, 0]])
            img = cv2.warpAffine(np.asarray(img), M, (img.width, img.height))
            img = Image.fromarray(img)

            if box_vis is not None:
                box_vis = F.box_translate(box_vis, x, img.width)
            
            ann = cv2.warpAffine(np.asarray(ann), M, (img.width, img.height)) if ann is not None else None
            ann = Image.fromarray(ann)
            
            if box_lwir is not None:
                box_lwir = F.box_translate(box_lwir, x, img.width)
...
class PI_RandomHorizontalShift(object):
    ...
    def __call__(self, img, ann=None, box_vis=None, box_lwir=None, pair=None):
        ...
        """
        centers, n_centers = 0, 1
        for bndbox in box_vis:
            if bndbox[0] == 0:
                continue
            
            centers += (bndbox[0]*img.width + bndbox[3]*img.width)/2
            n_centers += 1
        if n_centers != 0:
            centers /= n_centers
        if centers == 0 or centers >= img.width // 2:
            self.x *= -1
...
class TT_RandomShift(object):
    """Horizontally shift the given PIL Image randomly with a given probability.
    Args:
        p (float): probability of the image being translated. Default value is 0.5
        t (float or int): max shift pixel choosed randomly.
    """
    def __init__(self, p=0.5, tx=0, ty=0):
        self.p = p
        self.tx = tx
        self.ty = ty
    ...
    def __call__(self, img, ann=None, box_vis=None, box_lwir=None, pair=None):
        ...
        if random.random() < self.p:
            if self.tx < 0:
                tx = random.randint(self.tx, 0)
            else:
                tx = random.randint(0, self.tx)
            if self.ty < 0:
                ty = random.randint(self.ty, 0)
            else:
                ty = random.randint(0, self.ty)
            ...
            if box_vis is not None:
                box_vis = F.box_translate_xy(box_vis, tx, ty, img.width, img.height)
            
            ann = cv2.warpAffine(np.asarray(ann), M, (img.width, img.height)) if ann is not None else None
            ann = Image.fromarray(ann)
            
            if box_lwir is not None:
                box_lwir = F.box_translate_xy(box_lwir, tx, ty, img.width, img.height)
...
class TT_RandomZoomOut(object):
    """Zoom out operate (expand) the given PIL Image randomly with a given probability.
    Args:
        p (float): probability of the image being zoomed. Default value is 0.5
        s (float or int): max expand size
    """
    def __init__(self, p=0.5, s=4):
        self.p = p
        self.s = s
    ...
    def __call__(self, img, ann=None, box_vis=None, box_lwir=None, pair=None):
        """
        Args:
            img (PIL Image): Image to be zoomed.
        Returns:
            PIL Image: Randomly zoom out image.
        """
        if random.random() < self.p:
            img = torchvision.transforms.ToTensor(img)
            original_h = img.size(1)
            original_w = img.size(2)
