...
def make_logger(args):
    ...
    h = logging.FileHandler(os.path.join(args.exps_dir, 'log_{:s}.txt'.format(args.exp_time)))
    h.setFormatter(fmt)
    logger.addHandler(h)
...
def save_checkpoint(epoch, model, optimizer, loss, exps_dir):
    """
    ...
    :param exps_dir: save path
    """
    state = {'epoch': epoch,
             'loss': loss,
             'model': model,
             'optimizer': optimizer}
    filename = os.path.join(exps_dir,'checkpoint_insanet.pth.tar{:03d}'.format(epoch))
