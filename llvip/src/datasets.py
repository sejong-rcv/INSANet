...
class LLVIPPed(data.Dataset):
    """LLVIP Detection Dataset Object
    ...
    def __init__(self, args, condition='train'):
        ...
        self._imgpath = os.path.join('%s', '%s', '%s', '%s.jpg')  
            
        self.ids = list()
        
        for line in open(os.path.join(self.args.path.DB_ROOT, self.image_set)):
    ...
    def pull_item(self, index):
        ...
        cond, img_id = frame_id[-1]

        vis = Image.open( self._imgpath % ( *frame_id[:-1], 'visible', cond, img_id[:-4] ))
        lwir = Image.open( self._imgpath % ( *frame_id[:-1], 'infrared', cond, img_id[:-4] ) ).convert('L')
        ...
        if self.mode == 'train':
            ...
            for line in open(self._annopath % ( *frame_id[:-1], img_id[:-4] )) :
            ...
            for i in range(len(vis_boxes)) :
                ...
                bndbox.append(1) # class always 'person'
                boxes_vis += [bndbox]

            boxes_vis = np.array(boxes_vis, dtype=np.float)
            boxes_lwir = copy.deepcopy(boxes_vis)
