...
def val_epoch(model: INSANet, dataloader: DataLoader, input_size: Tuple, min_score: float = 0.1, epoch: int = 0) -> Dict:
    ...
    :param dataloader: Dataloader instance to feed training data(images, labels, etc) for LLVIP dataset
...
def run_inference(model_path: str) -> Dict:
    ...
    Load pretrained model and run inference on LLVIP dataset.
    ...
    test_dataset = LLVIPPed(args, condition="test")
